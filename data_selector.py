...
def search(bestObjID, wide_convert):
    sourceobjid = []
    ...
    for q in tqdm(range(len(bestObjID))):
        notfound = False
        j = np.where(( bestObjID[q] == wide_convert['bestObjID'] ))
    ...
    return drops, sourceobjid, plate, mjd, fiberid, u, g, r,i,z
...
def select_sdss4(el_badry = '../external-dat/binaries/all_columns_catalog.fits', outfile = 'data/catalog_sdss4.csv'):
    ...
    catalog = df[ df['binary_type'] == b'WDMS']
...
def old_select_sdss4(el_badry = '../external-dat/binaries/all_columns_catalog.fits', outfile = 'data/catalog_sdss4.csv'):
